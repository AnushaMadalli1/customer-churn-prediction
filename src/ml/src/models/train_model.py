...
def train_model(config):
    config = load_config()  # Ensure local config
    try:
    ...
    rf_tuned_params = config['models']['rf_tuned']['param_grid']
    ...
    rf_base_params = config['models']['rf']
    del rf_base_params['class']  # Remove invalid param
    lr_params = config['models']['lr']
    del lr_params['class']  # Remove invalid param
    rf_base_params['random_state'] = config['random_state']
    
    rf_base_mdl = RandomForestClassifier(**rf_base_params, random_state=config['random_state'])
    lr_mdl = LogisticRegression(**lr_params, random_state=config['random_state'])
    ...
    # Save tuned RF as best (improved)
    model_path = config['models']['model_path']
    os.makedirs(os.path.dirname(model_path), exist_ok=True)
    joblib.dump(tuned_rf, model_path)
    print(f"\nTuned RF saved to {model_path} (improved F1: {f1_rf:.3f} vs base)")
